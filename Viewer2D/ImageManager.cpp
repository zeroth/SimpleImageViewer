...
int ImageManager::addImage(const QString &path)
{
//    beginResetModel();
...
int ImageManager::addImage(const QUrl &val)
{
    return this->addImage(val.toLocalFile());
}

int ImageManager::rowCount(const QModelIndex &parent) const
{
    if(parent.isValid())
        return 0;
...
QHash<int, QByteArray> ImageManager::roleNames() const
{
    ...
    if(roles.isEmpty())
    {
        roles[BioImgName] = "bioImgName";
        roles[BioImgPath] = "bioImgPath";
        roles[BioImgIsTimeSeries] = "bioImgIsTimeSeries";
    }

    return roles;
}

zeroth::BioImage *ImageManager::imageAt(int row)
{
    ...
    if(!imageCache.contains(row))
    {
        ...
        imageCache[row] = new zeroth::BioImage(path);
    }
